            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                    echo '<p style="color: white;">' . $row["name"] .
                        '<a href="processar_gestor.php?user_id=' . $row["user_id"] . '&project_id=' . $_SESSION["project_id"] . '&delete=1" class="pull-right"><i class="fa fa-minus"></i></a>
                        <a href="processar_gestor.php?user_id=' . $row["user_id"] . '&project_id=' . $_SESSION["project_id"] . '&add=1" style="margin-right: 1rem;" class="pull-right"><i class="fa fa-plus"></i></a></p>';
